...
from boligscan_core import (
    ...
    geocode_address,
    get_distance,
    finn_naermeste_skole,
    SKOLER,
    get_alert_key,
    bolig_matcher_alert,
    maybe_send_slack_alert,
    scrape_finn,
    get_gmail_service,
    get_email_body,
    ...
    lagre_salgsoppgave_forsok,
    analyser_og_lagre_dokumentanalyse,
    hent_salgsoppgave_med_broker_fallback,
)
...
from document_parser import COMPONENT_ORDER, COMPONENT_DISPLAY_NAVN
...
# ---------------- AUTOMATISK BAKGRUNNSJOBB ----------------
# Den daglige boligscanen (Gmail -> FINN -> lagring -> Slack) og
# Gmail-helsesjekken kjøres IKKE lenger fra denne Streamlit-prosessen.
# Streamlit kjører aldri skriptets kode før en nettleser-økt kobler seg til
# (bekreftet direkte i praksis - verken en kjørende container, et rått
# HTTP-kall eller en rå WebSocket-tilkobling trigget noe scan), så en
# planlegger her ville aldri kjørt autonomt uten at noen åpner appen i en
# nettleser. Dette gjøres nå av worker.py - en egen, alltid-på
# bakgrunnsprosess i samme Docker-container (se docker-compose.yml) som
# importerer nøyaktig samme kjor_planlagt_boligscan() fra boligscan_core.py.
# Denne Streamlit-appen bruker den samme delte kjernelogikken kun for de
# MANUELLE knappene nedenfor.
...
if _gmail_status is None:
    ...
    if _gmail_status["krever_reautentisering"]:
        st.sidebar.warning(
            "⚠️ Gmail-tokenet er ugyldig eller utløpt. Kjør `python "
            "gmail_test.py` PÅ VERTEN (utenfor Docker, i en vanlig terminal "
            "med nettleser) for å autentisere på nytt - se gmail_test.py sin "
            "docstring. Det nye token.json plukkes opp automatisk av både "
            "boligscanner og boligscanner-worker, uten restart."
        )
...
# lagre_salgsoppgave_forsok, finn_bolig_id_for_finn_annonse,
# oppdater_bolig_broker_og_salgsoppgave, analyser_og_lagre_dokumentanalyse og
# hent_salgsoppgave_med_broker_fallback flyttet til boligscan_core.py (se
# import øverst i filen) slik at den automatiske scanen (worker.py, som ikke
# importerer app.py) også kan hente og analysere salgsoppgaven for hver nye
# bolig - ikke bare de manuelle knappene under.
...
st.caption(
    "Kjøres også automatisk hver dag kl. 08:00 (Europe/Oslo) av en separat "
    "bakgrunnsprosess (worker.py, se docker-compose.yml) - helt uavhengig av "
    "om denne appen er åpen i en nettleser. Denne knappen er for å hente "
    "umiddelbart med gjeldende sidebar-innstillinger."
)

siste_scan = siste_boligscan_dato()
st.caption(f"Siste vellykkede scan (manuell eller automatisk): {siste_scan or 'ingen registrert ennå'}")

if st.button("Hent nye FINN-varsler fra Gmail"):
    try:
        resultat = kjor_gmail_boligscan(alert_settings, rente, nedbetaling_ar, ek_prosent, bruk_makslaan, maks_laan)
        logg_boligscan("manual", resultat)
        st.success(
            f"Ferdig: {resultat['nye']} nye boliger lagret, {resultat['allerede']} fantes fra før, "
            f"{resultat['feilet']} feilet, {resultat['varslet']} Slack-varsler sendt."
        )
    except Exception as e:
        logg_boligscan("manual", None, error_message=str(e))
...
def bygg_dashboard_synk_poster(rente, nedbetaling_ar, ek_prosent, bruk_makslaan, maks_laan):
    ...
    SELECT id, url, adresse, postnummer, by, pris, felleskost, soverom, leie, strom, kommunale, andre,
           ...
           nearest_school, nearest_school_km, nearest_school_min,
